...
def saturator(teststream, name="water saturator"):
    streamsaturator = neqsim.processSimulation.processEquipment.util.StreamSaturatorUtil(
        teststream)
...
def filters(teststream):
    filter2 = neqsim.processSimulation.processEquipment.filter.Filter(
        teststream)
...
def simplereservoir(fluid, name="Reservoir 1",  gasvolume=10.0 * 1e7, oilvolume=120.0 * 1e6, watervolume=10.0e6):
    reserv = neqsim.processSimulation.processEquipment.reservoir.SimpleReservoir(
        name)
...
def distillationColumn(trays=5, reboil=True, condenser=True, name="destColumn"):
    distillationColumn = neqsim.processSimulation.processEquipment.distillation.DistillationColumn(
