...
import pandas as pd
from neqsim.thermo import (TPflash, calcproperties, fluid, fluid_df,
...
# Define a function to calculate properties of the fluid at equilibrium at given temperature and pressure
def calcProperties(frame):
    ...
    if gascondensateFluid.hasPhaseType("gas"):
        ...
        frame['gasmolfraction[mol/mol]'] = gascondensateFluid.getMoleFraction(
            phaseNumber)
        frame['gasvolumefraction[mol/mol]'] = gascondensateFluid.getCorrectedVolumeFraction(
            phaseNumber)
        frame['gasthermalconductivity[W/mK]'] = gascondensateFluid.getPhase(
            phaseNumber).getThermalConductivity("W/mK")
        frame['gasZ[-]'] = gascondensateFluid.getPhase(phaseNumber).getZ()
        frame['gasmolarMass[kg/mol]'] = gascondensateFluid.getPhase(
            phaseNumber).getZ()
        frame['gasenthalpy[kg/mol]'] = gascondensateFluid.getPhase(
            phaseNumber).getEnthalpy("J/mol")
        frame['gasviscosity[kg/msec]'] = gascondensateFluid.getViscosity(
            "kg/msec")
    ...
    if gascondensateFluid.hasPhaseType("oil"):
        ...
        frame['oilmolfraction[mol/mol]'] = gascondensateFluid.getMoleFraction(
            phaseNumber)
        frame['oilvolumefraction[mol/mol]'] = gascondensateFluid.getCorrectedVolumeFraction(
            phaseNumber)
        frame['oilthermalconductivity[W/mK]'] = gascondensateFluid.getPhase(
            phaseNumber).getThermalConductivity("W/mK")
        frame['oilZ[-]'] = gascondensateFluid.getPhase(phaseNumber).getZ()
        frame['oilmolarMass[kg/mol]'] = gascondensateFluid.getPhase(
            phaseNumber).getZ()
        frame['oilenthalpy[kg/mol]'] = gascondensateFluid.getPhase(
            phaseNumber).getEnthalpy("J/mol")
        frame['oilviscosity[kg/msec]'] = gascondensateFluid.getViscosity(
            "kg/msec")
        frame['oilmolfraction[mol/mol]'] = gascondensateFluid.getMoleFraction(
            phaseNumber)
        frame['oilvolumefraction[mol/mol]'] = gascondensateFluid.getCorrectedVolumeFraction(
...
# Create list of 1000 random tempeatures between 0 and 50 deg C
temperatures_list = [random.uniform(273.15, 400.0) for i in range(1000)]
# Create list of  1000 random pressures between 1 and 450 bara
pressures_list = [random.uniform(1.0, 450.0) for i in range(1000)]

temppresdict = {'temperature':  temperatures_list,
                'pressure':  pressures_list
                }

# Method 1: Creating a dataframe applying method calcProperties
propertiesdf1 = pd.DataFrame(temppresdict)
propertiesdf1 = propertiesdf1.apply(calcProperties, axis=1)
print(propertiesdf1)


# Method 2: Creating a dataframe with properties using the calcproperties function.
propertiesdf2 = calcproperties(gascondensateFluid, temppresdict)
print(propertiesdf2)
