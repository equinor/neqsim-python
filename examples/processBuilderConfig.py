...
config = {
    ...
    "equipment": [
        ...
        {"type": "separator", "name": "inlet_separator", "inlet": "inlet"},
        {
            "type": "compressor",
...
print(
    f"Stage 1 outlet pressure: {process.get('stage1_compressor').getOutletStream().getPressure():.1f} bara"
)
print(f"Stage 1 power: {process.get('stage1_compressor').getPower()/1e3:.1f} kW")
print(
    f"Stage 2 outlet pressure: {process.get('stage2_compressor').getOutletStream().getPressure():.1f} bara"
)
print(f"Stage 2 power: {process.get('stage2_compressor').getPower()/1e3:.1f} kW")
print(
    f"Total power: {(process.get('stage1_compressor').getPower() + process.get('stage2_compressor').getPower())/1e3:.1f} kW"
)
...
try:
    ...
    process2 = ProcessBuilder.from_json(
        json_file, fluids={"reservoir_fluid": reservoir_fluid}
    ).run()

    print("\n=== Separator Process Results (from JSON) ===")
    print(
        f"HP Separator gas rate: {process2.get('hp_separator').getGasOutStream().getFlowRate('MSm3/day'):.2f} MSm3/day"
    )
    print(
        f"LP valve outlet pressure: {process2.get('lp_valve').getOutletStream().getPressure():.1f} bara"
    )
finally:
...
"""
    ...
    try:
        ...
        print(
            f"Cooler outlet temperature: {process3.get('cooler1').getOutletStream().getTemperature() - 273.15:.1f} °C"
        )
        print(
            f"Export compressor power: {process3.get('export_compressor').getPower()/1e3:.1f} kW"
        )
    finally:
        os.unlink(yaml_file)
