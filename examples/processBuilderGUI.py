...
COMPONENT_OPTIONS = [
    ...
    'nitrogen', 'CO2', 'H2S', 'water', 'H2', 'oxygen'
]
...
with tab1:
    ...
    with col1:
        ...
        for i, comp in enumerate(COMPONENT_OPTIONS[:10]):
            with cols[i % 2]:
                val = st.number_input(comp, min_value=0.0, max_value=1.0, 
                                     value=0.9 if comp == 'methane' else 0.1 if comp == 'ethane' else 0.0,
                                     step=0.01, key=f"comp_{comp}")
                if val > 0:
                    components[comp] = val
        ...
        if st.session_state.fluids:
            for name, config in st.session_state.fluids.items():
                with st.expander(f"📦 {name}", expanded=True):
                    ...
                    st.write(f"**T:** {config['temperature']} °C | **P:** {config['pressure']} bara")
                    if config.get('flow_rate'):
                        st.write(f"**Flow:** {config['flow_rate']} {config['flow_unit']}")
...
with tab2:
    ...
    with col1:
        ...
        if EQUIPMENT_TYPES[eq_type]['has_fluid']:
            if st.session_state.fluids:
                params['fluid'] = st.selectbox("Select Fluid", list(st.session_state.fluids.keys()))
            else:
        ...
        if eq_type == 'compressor':
            params['pressure'] = st.number_input("Outlet Pressure (bara)", value=100.0, min_value=0.1)
            ...
            params['pressure'] = st.number_input("Outlet Pressure (bara)", value=10.0, min_value=0.1)
        
        elif eq_type == 'valve':
            params['pressure'] = st.number_input("Outlet Pressure (bara)", value=10.0, min_value=0.1)
        
        elif eq_type == 'heater':
            params['temperature'] = st.number_input("Outlet Temperature (K)", value=350.0)
        
        elif eq_type == 'cooler':
            params['temperature'] = st.number_input("Outlet Temperature (K)", value=300.0)
    ...
    with col2:
        ...
        if st.session_state.equipment_list:
            for i, eq in enumerate(st.session_state.equipment_list):
                with st.expander(f"{'🔵' if eq['type'] == 'stream' else '⚙️'} {eq['name']} ({eq['type']})", expanded=True):
                    ...
                    with col_a:
                        if st.button("⬆️ Move Up", key=f"up_{i}", disabled=i==0):
                            st.session_state.equipment_list[i], st.session_state.equipment_list[i-1] = \
                                st.session_state.equipment_list[i-1], st.session_state.equipment_list[i]
                            st.rerun()
                    with col_b:
            ...
            st.info("No equipment added yet. Use the form on the left to add equipment.")
                ...
                if eq_obj:
                    with st.expander(f"📋 {eq['name']} Details"):
                        ...
                        if out_stream:
                            ...
                            with col1:
                                st.metric("Temperature", f"{out_stream.getTemperature() - 273.15:.1f} °C")
                            with col2:
                                st.metric("Pressure", f"{out_stream.getPressure():.1f} bara")
                            with col3:
                                try:
                                    st.metric("Flow Rate", f"{out_stream.getFlowRate('kg/hr'):.0f} kg/hr")
                                except:
                                    pass
