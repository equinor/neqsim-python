...
Vtmax = Ks * math.sqrt(
    (
        separator1.getThermoSystem().getPhase("oil").getDensity()
        - separator1.getThermoSystem().getPhase("gas").getDensity()
    )
    / separator1.getThermoSystem().getPhase("gas").getDensity()
)
diameter = math.sqrt(
...
flowratewater = (
    coolerDuty / (enthalpy1 - enthalpy2) * fluid2.getMolarMass() * 3600 / 1000.0
)
