...
def test_dynamic_compressor():
    testSystem2 = neqsim.thermo.system.SystemSrkEos((273.15 + 25.0), 10.00)
    ...
    separator1 = neqsim.processsimulation.processequipment.separator.Separator(
        "separator_1"
    )
    ...
    compressor1 = neqsim.processsimulation.processequipment.compressor.Compressor(
        "comp1", separator1.getGasOutStream()
    )
    compressor1.setCalculateSteadyState(False)
    compressor1.setOutletPressure(100.0)

    separator2 = neqsim.processsimulation.processequipment.separator.Separator(
        "separator_2"
    )
    ...
    valve2 = neqsim.processsimulation.processequipment.valve.ThrottlingValve(
    ...
    p = neqsim.processsimulation.processsystem.ProcessSystem()
    p.add(stream1)
    p.add(valve1)
    p.add(separator1)
    p.add(compressor1)
