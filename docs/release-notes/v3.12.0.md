...
- Bundles the NeqSim Java 3.12.0 engine for Java 11+ runtimes.
- Bundles the NeqSim Java 3.12.0 engine for Java 8 runtimes.
- Aligns Python package metadata and the conda recipe with version 3.12.0.
...
```bash
pip install --upgrade neqsim==3.12.0
```

Use conda when you want OpenJDK installed with the package:

```bash
conda install -c conda-forge neqsim=3.12.0
```
...
- Full changelog: https://github.com/equinor/neqsim-python/compare/v3.11.0...v3.12.0
